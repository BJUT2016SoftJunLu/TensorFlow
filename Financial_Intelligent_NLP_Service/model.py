...
class TextCNN(object):
    ...
    def __init__(self, max_document_length, vocab_size,embedding_size,
        ...
        self.input_x = tf.placeholder(tf.int32,shape=[None,max_document_length])
        self.input_y = tf.placeholder(tf.float32,shape=[None,class_nums])
        ...
        self.prob = tf.nn.softmax(conn_output)
        self.accuracy =  tf.reduce_mean(tf.cast(tf.equal(tf.argmax(conn_output,1),tf.argmax(self.input_y,1)),"float"))
